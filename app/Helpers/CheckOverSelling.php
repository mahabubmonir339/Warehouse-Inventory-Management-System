...
{
    public function check($order_items, $warehouse_id, $was_draft = false)
    {
        $error = [];
        ...
        foreach ($order_items as $key => $order_item) {
            ...
            if (isset($order_item['selected']) && isset($order_item['selected']['variations']) && ! empty($order_item['selected']['variations'])) {
                foreach ($order_item['selected']['variations'] as $order_variation) {
                    ...
                    $old_quantity = $was_draft ? 0 : (isset($order_variation['old_quantity']) ? (float) $order_variation['old_quantity'] : 0);
                ...
                $old_quantity = $was_draft ? 0 : (isset($order_item['old_quantity']) ? (float) $order_item['old_quantity'] : 0);
                $quantity = $old_quantity ? $quantity - $old_quantity : $quantity;
